...
class TableHeaderColumn extends Component {
	...
	getFilters () {
		...
		case Const.FILTER_TYPE.TEXT: {
			return <TextFilter ref='textFilter' {...this.props.filter} columnName={this.props.children} filterHandler={this.handleFilter} />
		}
		case Const.FILTER_TYPE.REGEX: {
			return <RegexFilter ref='regexFilter' {...this.props.filter} columnName={this.props.children} filterHandler={this.handleFilter} />
		}
		case Const.FILTER_TYPE.SELECT: {
			return <SelectFilter ref='selectFilter' {...this.props.filter} columnName={this.props.children} filterHandler={this.handleFilter} />
		}
		case Const.FILTER_TYPE.NUMBER: {
			return <NumberFilter ref='numberFilter' {...this.props.filter} columnName={this.props.children} filterHandler={this.handleFilter} />
		}
		case Const.FILTER_TYPE.DATE: {
			return <DateFilter ref='dateFilter' {...this.props.filter} columnName={this.props.children} filterHandler={this.handleFilter} />
		}
		case Const.FILTER_TYPE.CUSTOM: {
			const elm = this.props.filter.getElement(this.handleFilter, this.props.filter.customFilterParameters)
	...
	render () {
		...
		if (sortIndicator) {
			defaultCaret = (!dataSort) ? <span className='padding-15'></span> : (
		...
		const classes = classSet(typeof className === 'function' ? className() : className, dataSort ? 'sort-column' : '')

		const title = headerTitle && typeof children === 'string' ? { title: children } : null
		return (
			<th ref='header-col' className={classes} style={thStyle} onClick={this.handleColumnClick.bind(this)} {...title}>
				{ sortCaret } { children }
	...
	renderReactSortCaret (order) {
		...
		return (
			<span className={orderClass}>
				<span className='sortCaret active' />
			</span>
		)
	}
