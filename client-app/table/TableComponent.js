	...
	initTable (props) {
		...
		React.Children.forEach(props.children, column => {
			...
			if (column.props.filter) {
				...
				if (!this.filter) {
					// first time create the filter on the TableComponent
		...
		if (!isKeyFieldDefined && !keyField) {
			throw new Error(`Error. No any key column defined in TableHeaderColumn. Use 'isKey={true}' to specify a unique column after version 0.5.4.`)
		}
	...
	*/
			...
			<div className={classSet('react-bs-table-container', this.props.containerClass)}
				...
				<div ref='table'
					...
					<TableBody ref='body'
						...
						adjustHeaderWidth={this._adjustHeaderWidth.bind(this)} />
				</div>
				{ tableFilter }
				{ pagination }
			</div>
	...
	handleSelectAllRow (e) {
		...
		if (typeof result === 'undefined' || result !== false) {
			if (isSelected) {
				selectedRowKeys = Array.isArray(result) ? result
	...
	handleDropRow (rowKeys = this.store.getSelectedRowKeys()) {
		const dropRowKeys = rowKeys
	...
	renderTableFilter (columns) {
		if (this.props.columnFilter) {
			return <TableFilter columns={columns} rowSelectType={this.props.selectRow.mode} onFilter={this.handleFilterData.bind(this)} />
		} else {
			return null
		}
	}
...
TableComponent.propTypes = {
	...
	cellEdit: PropTypes.shape({
		...
		__onCompleteEdit__: PropTypes.func
	}),
