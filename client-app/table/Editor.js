...
const editor = function (editable, attr, format, editorClass, defaultValue, ignoreEditable) {
	if (editable === true || (editable === false && ignoreEditable) || typeof editable === 'string') { // simple declare
		...
		if (editable.type === 'select') { // process select input
			...
			return (
				<div>
					<textarea {...attr} defaultValue={defaultValue} />
					{ saveBtn }
				</div>
			)
			...
			const checked = defaultValue && defaultValue.toString() === values.split(':')[0]
			...
			return <input {...attr} type='datetime-local' defaultValue={defaultValue} />
		} else { // process other input type. as password,url,email...
			return <input {...attr} type='text' defaultValue={defaultValue} />
		}
	}
