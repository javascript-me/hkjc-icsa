...
describe('Page Componente', () => {
	...
	it('Shoud throw an error', () => {
		...
		try{
			const cpage = mount(<PageComponent />)
		} catch(e) {
		...
		try{
			const cpage = mount(<PageComponent>
				<PageLayer typeLayer="body">
					<TableHeaderColumn dataField='id' autoValue={true} isKey={true}>ID</TableHeaderColumn>
	...
	it('Render Page with Table and Data ', () => {
		const options = {
			table: {
				method: 'GET',
				endpoint: config.API_BROADCAST_SEARCH,
				options: {}
			}
		}
	...
	it('Render Page with Table and Filters ', () => {
		...
		const cpage = mount(<PageComponent options={options} tableData={data}>
			<PageLayer typeLayer="body">
				<TableHeaderColumn dataField='id' autoValue={true} isKey={true}>ID</TableHeaderColumn>
				<TableHeaderColumn dataField='distribution_date' dateRange={true} isFilter={true}>Distribution Date & Time</TableHeaderColumn>
				<TableHeaderColumn dataField='name' isFilter={true}>Name</TableHeaderColumn>
				<TableHeaderColumn dataField='category' isFilter={true} filterOptions={ {ctrlType:'select', dataSource: [{id:1,value:'option'},{id:2,value:'option'}]} }>Category</TableHeaderColumn>
				<TableHeaderColumn dataField='detail'>Detail</TableHeaderColumn>
			</PageLayer>
		</PageComponent>)
